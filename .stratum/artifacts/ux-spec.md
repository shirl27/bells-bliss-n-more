...
| Call to Book (`tel:+12063217416`) / Email (`mailto:`) | Catalog, Planning Services: one consolidated CTA at the bottom of the page (not per card). Item Detail: one CTA for the single item shown, with the item name in the email subject line | Phase 0 only | Replaces Add to Cart/Booking entirely — opens the phone dialer or the visitor's mail client; no cart state, no toast. Per-card CTAs were considered and rejected on Catalog/Planning Services: a `tel:` link carries no per-item context regardless of which card triggers it, so repeating an identical action once per card was pure redundancy, not a real per-item feature — see `phase0-mvp-criteria.md` §3 |
| View Details | Catalog | E2E, MVP only | Excluded from Phase 0 — it links to `item-detail.html`, which always shows one hardcoded item regardless of which card was clicked (a pre-existing prototype limitation). That's tolerable in E2E/MVP since "Add to Cart" already works correctly per item without needing this link; in Phase 0 it would have been the only action per card, making the mismatch load-bearing. Removed rather than fixed, since the card already shows name/description/price, which is what Phase 0 needs |
...
| Control | Screen | Applies to | Behavior |
|---|---|---|---|
| Newsletter signup | Home footer | E2E, MVP, Phase 0 | Prototype-only, no real subscription anywhere. Submitting hides the form and reveals a "Thanks — you're on the list!" confirmation (`submitNewsletterSignup()`) — unlike the mailto controls above, there's no external system this could fail to reach, so showing a confirmation doesn't claim anything false; it's consistent with every other simulated interaction in the prototype (e.g. Add to Cart's toast) |
| Request Info form | Home | Phase 0 only | Name/email/message fields; submit builds a `mailto:info@bellsblissnmore.com` URL with the fields URL-encoded into the subject/body and navigates to it via `window.location.href` (`submitRequestInfo()` in `prototype-behavior.js`) — zero backend, opens the visitor's own mail client. There's no JS API to detect whether that handoff actually succeeded, so rather than trying to react to failure, submitting always reveals a fallback block (`#ri-confirmation`) showing the direct phone/email and the composed message in a read-only textarea — a permanent safety net, not an error state |
| Call to Book / Email CTA buttons | Catalog, Planning Services, Item Detail | Phase 0 only | Same undetectable-failure issue as the Request Info form applies here, but these are plain links with no JS hook to react from. Fix is structural instead: the actual phone number and email address are shown as plain visible text directly below the buttons, so the visitor always has what they need to act manually regardless of whether the `tel:`/`mailto:` handoff does anything. (The footer's contact links didn't need this — the link text there already *is* the phone number/email, unlike a generic "Call to Book" label) |
